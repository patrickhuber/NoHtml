...
namespace NoHtml.Web.Parsing
{
    public class StateMachine
    {
        ...
        private readonly Dictionary<int, List<Transition>> states;
        ...
        private class RegexTransition : Transition
        {
            ...
            public override bool IsMatch(Token token)
            {
                return base.IsMatch(token) && Regex.IsMatch(token.Data, Pattern);
            }
        }
        ...
        public bool MoveNext(Token token)
        {
            ...
            if (!states.TryGetValue(CurrentState, out transitions))
                return false;
            foreach (var transition in transitions.Where(transition => transition.IsMatch(token)))
            {
